...
function CadastroTarefa(props) {
  ...
  const handleSubmit = async(e) => {
    ...
    try {
        ...
        await axios.post('/tarefas', { titulo, descricao, userId })
                ...
                <Grid container spacing={24}>               
                    <Grid item xs={12} md={6}>
                        <FormControl margin='normal'  fullWidth>
                            ...
                            <Input id='titulo' name='titulo' type='text' value={titulo} onChange={({ target }) => setTitulo(target.value)} autoComplete='titulo' autoFocus />
                        </FormControl>
                    </Grid>
                    <Grid item xs={12} md={6}>
                        <FormControl margin='normal'  fullWidth>
                            <InputLabel htmlFor='descricao'>Descrição</InputLabel>
                            <Input id='descricao' name='descricao' type='text' value={descricao} onChange={({ target }) => setDescricao(target.value)} autoComplete='descricao' />
                        </FormControl>
                    </Grid> 
...
CadastroTarefa.propTypes = {
  classes: PropTypes.object.isRequired,
}
export default withStyles(styles)(CadastroTarefa)